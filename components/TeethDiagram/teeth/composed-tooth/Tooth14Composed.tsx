...
function Tooth14Composed({ customStyles }: ComposesToothProps) {
  const { treatments } = useTeethDiagramStore((state) => state);
  const toothData = treatments['14' as keyof typeof treatments] || {};

  return (
    <ToothContainer tooth={14} customStyles={customStyles}>
      <Tooth tooth={14} variant={toothData.toothVariant}>
        <Root tooth={14} variant={toothData.rootVariant} />
        <Crown
          tooth={14}
      ...
      <ToothNumber tooth={14} jawType={JawType.UPPER} />
    </ToothContainer>
  );
}

export default Tooth14Composed;
